...
\begin{frame}[label={rules}]
	...
	\begin{tabular}{r|l|l}
		...
		\onslide<11->CN & Body  & CHR Constraints and conjunction built-ins
	\end{tabular}\onslide<1->
	\ul<10->{gray}{bu1s}{bu1e}{bi}\begin{tikzpicture}[o]
		\node[shift={(26mm,5mm)},visible on=<10->] at (bi) (BI) {\small\color{darkgray}Zig expressions};
		\draw[textarrow,shorten <=0,visible on=<10->] (BI.west) to (bi_east);
	\end{tikzpicture}
	\begin{modal}<6>[Generalized Simpagation Rule]
	...
	\end{modal}
	\onslide<6>\begin{tikzpicture}[o,scale=0.5]
